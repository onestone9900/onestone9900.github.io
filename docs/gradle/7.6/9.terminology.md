...
## 기능 변형(Feature Variant)
기능 변형은 개별적으로 선택하거나 선택하지 않을 수 있는 컴포넌트의 기능을 나타내는 [변형](/docs/gradle/7.6/9.terminology/#%EA%B8%B0%EB%8A%A5-%EB%B3%80%ED%98%95feature-variant)이다. 기능 변형은 하나 이상의 기능(capabilities)으로 식별된다. 자세한 내용은, 기능 변형 및 선택적 의존성 모델링 절을 참고하자.
