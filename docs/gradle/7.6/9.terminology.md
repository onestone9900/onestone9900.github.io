...
## 리포지터리(Repository)
리포지터리는 모듈 세트를 호스팅하며, 각 모듈은 모듈 버전으로 표시된 하나 이상의 릴리스(컴포넌트)를 제공할 수 있다. 리포지터리는 바이너리 리포지터리 제품(예: Artifactory 또는 Nexus) 또는 파일 시스템의 디렉토리 구조를 기반으로 할 수 있다. 자세한 내용은 리포지토리 선언을 참고하자.
