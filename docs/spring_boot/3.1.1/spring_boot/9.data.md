...
### 9.2.7. LDAP
LDAP(Lightweight Directory Access Protocol)는 IP 네트워크를 통해 분산된 디렉터리 정보 서비스에 접근하고 유지 관리하기 위한 개방형, 공급업체 중립적, 산업 표준 응용 프로그램 프로토콜이다. 스프링 부트는 호환되는 모든 LDAP 서버에 대한 자동 구성을 제공할 뿐만 아니라 UnboundID의 임베디드 인메모리 LDAP 서버에 대한 지원도 제공한다.
