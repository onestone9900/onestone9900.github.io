...
`/var/myapp`에 스프링 부트 애플리케이션이 설치되어 있다고 가정하고 스프링 부트 애플리케이션을 `systemd` 서비스로 설치하려면 `myapp.service`라는 스크립트를 생성하여 `/etc/systemd/system` 디렉터리에 배치한다. 다음은 예제 스크립트다.
...
A([별도 유지 관리 샘플](https://github.com/snicoll/spring-boot-daemon))에서는 스프링 부트 애플리케이션용 Windows 서비스를 만드는 방법을 단계별로 설명한다.