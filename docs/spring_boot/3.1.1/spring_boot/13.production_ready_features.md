...
`io.micrometer.core.annotation` 패키지의 `@Timed` 어노테이션은 리포지토리 인터페이스 및 메서드에서 지원된다. 모든 리포지토리 호출에 대한 지표를 기록하지 않으려면 `management.metrics.data.repository.autotime.enabled`를 `false`로 설정하고 대신 `@Timed` 어노테이션을 독점적으로 사용할 수 있다.

{: .note}
`longTask = true`인 `@Timed` 어노테이션은 해당 메서드에 대해 긴 작업 타이머를 활성화한다. 긴 작업 타이머에는 별도의 메트릭명이 필요하며 짧은 작업 타이머와 함께 누적될 수 있다.

기본적으로, 리포지토리 호출 관련 메트릭에는 다음 정보가 태그로 지정된다.
