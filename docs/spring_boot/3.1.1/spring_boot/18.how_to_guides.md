...
### 18.8.7. 하이버네이트 2파 레벨 캐싱 구성(Configure Hibernate Second-Level Caching)
...
외부 구성에서 자동 구성된 리포지터리를 켜고 끄는 데 사용할 수 있는 플래그(`spring.data.*.repositories.enabled` 및 `spring.data.*.repositories.type`)도 있다. 예를 들어 몽고 리포지토리를 끄고 자동 구성된 `MongoTemplate`을 사용하려는 경우 이렇게 하는것이 유용하다.
