...
네임스페이스를 사용하지 않는 경우, AOP를 사용하여 리포지터리의 트랜잭션 동작도 구성해야 합니다. 다음 예는 XML에서 리포지터리의 트랜잭션 동작을 구성하는 방법을 보여준다:
