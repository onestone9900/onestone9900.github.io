...
`잡익스큐션(JobExecution)` 인스턴스가 반환되는 방식에 대한 정확한 동작은 제공된 `태스크익스큐터(TaskExecutor)`에 따라 다르다. 동기(승글 스레드) `태스트익스큐터(TaskExecutor)` 구현체가 사용되는 경우 `잡익스큐션(JobExecution)` 응답은 작업이 완료된 후에만 반환된다. 비동기 `태스크익스큐터(TaskExecutor)`를 사용하면, `잡익스큐션(JobExecution)` 인스턴스가 즉시 반환된다. 그런 다음 `잡익스큐션(JobExecution)` 인스턴스의 ID(JobExecution.getJobId() 사용)를 가져오고 `잡익스플로러(JobExplorer)`를 사용하여 `잡리포지터리(JobRepository)`에서 잡의 업데이트된 상태를 쿼리할 수 있다. 자세한 내용은 [리포지터리 쿼리](https://onestone9900.github.io/docs/spring_batch/5.0.2/5.configuring_and_running_a_job/#461-querying-the-repository)를 참고하자.
