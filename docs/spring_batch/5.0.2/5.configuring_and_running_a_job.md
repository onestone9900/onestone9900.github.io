...
다음 예에서는 XML에서 아이솔레이션 레벨을 재정의하는 방법을 보여준다:
...
다음 예에서는 자바에서 아이솔레이션 레벨을 재정의하는 방법을 보여준다:
...
빈 포스트 프로세서(bean post processor)를 사용하거나 레지스트러 라이프사이클 컴포넌트(registrar lifecycle component)를 사용하여 `잡레지스트리(JobRegistry)`를 채울 수 있다. 다음 절에서는 이 두 가지 메커니즘에 대해 설명한다.
