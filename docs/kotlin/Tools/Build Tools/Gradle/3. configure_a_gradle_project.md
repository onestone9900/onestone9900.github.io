...
코틀린 소스와 자바 소스는 ​​동일한 디렉터리에 저장하거나, 다른 디렉터리에 배치할 수 있다.

기본 컨벤션은 다른 디렉터리를 사용하는 것이다:
...
{: .warning }
>자바```.java``` 파일은 컴파일되지 않으므로, ```src/*/kotlin``` 디렉터리에 ```.java``` 파일을 저장하지 말자. 대신, ```src/main/java```를 사용할 수 있다.
...
{: .important}
>일반적이게, module-info.java를 src/main/java 디렉터리에 넣는다.
